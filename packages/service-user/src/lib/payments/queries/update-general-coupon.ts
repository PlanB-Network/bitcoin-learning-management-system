import { sql } from '@blms/database';

export const updateGeneralCoupon = ({ paymentId }: { paymentId: string }) => {
  return sql`
    UPDATE content.coupon_code
    SET uses = uses + 1
    FROM users.general_payment
    WHERE
      content.coupon_code.deleted_at IS NULL AND
      content.coupon_code.code = users.course_payment.coupon_code AND
      users.general_payment.coupon_code IS NOT NULL AND
      users.general_payment.payment_id = ${paymentId}
  ;
    `;
};
