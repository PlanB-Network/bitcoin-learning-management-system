...
import {
  ...
  isTeacherAnnouncementType,
} from '#src/utils/notifications.tsx';
...
export const NotificationsPanel = ({
  ...
  const handleCloseNotification = async (id: string) => {
    ...
    try {
      ...
      await trpcClient.user.notifications.markUserNotificationsAsRead.mutate({
        notificationIds: [id],
      });
  ...
  return (
    <>
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className={cn(
              'max-lg:hidden group z-50 flex size-[52px] items-center justify-center gap-2 rounded-2xl text-sm font-semibold outline-hidden transition-all hover:bg-darkOrange-4 hover:dark:bg-newBlack-3 lg:gap-2.5 relative',
              className,
      ...
      {/* Mobile - Button only (no dashboard link) */}
      <button
        type="button"
